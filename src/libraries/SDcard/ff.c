...
#define	DEF_NAMEBUF			UCHAR sfn[12]
...
#define	DEF_NAMEBUF			UCHAR sfn[12]; WCHAR lbuf[_MAX_LFN+1]
#define INIT_BUF(dobj)		{ (dobj).fn = sfn; (dobj).lfn = lbuf; }
#define	FREE_BUF()

#elif _USE_LFN == 3 		/* LFN with dynamic LFN working buffer on the heap */
#define	DEF_NAMEBUF			UCHAR sfn[12]; WCHAR *lfn
...
#if _USE_LFN
static
const UCHAR LfnOfs[] = {1,3,5,7,9,14,16,18,20,22,24,28,30};	/* Offset of LFN chars in the directory entry */


static
int cmp_lfn (			/* 1:Matched, 0:Not matched */
	WCHAR *lfnbuf,		/* Pointer to the LFN to be compared */
	UCHAR *dir			/* Pointer to the directory entry containing a part of LFN */
)
{
	UINT i, s;
	WCHAR wc, uc;
...
int pick_lfn (			/* 1:Succeeded, 0:Buffer overflow */
	...
	UCHAR *dir			/* Pointer to the directory entry */
)
{
	UINT i, s;
	WCHAR wc, uc;
...
#if _USE_LFN
void gen_numname (
	UCHAR *dst,			/* Pointer to generated SFN */
	const UCHAR *src,	/* Pointer to source SFN to be modified */
...
static
UCHAR sum_sfn (
	const UCHAR *dir		/* Ptr to directory entry */
)
{
	UCHAR sum = 0;
	UINT n = 11;
	...
	if (res == FR_OK && ne > 1) {	/* Initialize LFN entry if needed */
		...
		if (res == FR_OK) {
			...
			do {					/* Store LFN entries in bottom first */
				res = move_window(dj->fs, dj->sect);
				if (res != FR_OK) break;
				fit_lfn(dj->lfn, dj->dir, (UCHAR)ne, sum);
				dj->fs->wflag = 1;
				res = dir_next(dj, 0);	/* Next entry */
			} while (res == FR_OK && --ne);
		}
...
{
#ifdef _EXCVT
	static const UCHAR excvt[] = _EXCVT;	/* Upper conversion table for extended chars */
#endif
...
static
UCHAR check_fs (	/* 0:The FAT BR, 1:Valid BR but not an FAT, 2:Not a BR, 3:Disk error */
	FATFS *fs,	/* File system object */
...
FRESULT chk_mounted (	/* FR_OK(0): successful, !=0: any error occurred */
	...
	UCHAR chk_wp			/* !=0: Check media write protection for write access */
)
{
	UCHAR fmt, b, *tbl;
	UINT vol;
	...
	*rfs = fs = FatFs[vol];				/* Return pointer to the corresponding file system object */
	...
	fs->drv = (UCHAR)LD2PD(vol);			/* Bind the logical drive and a physical drive */
...
FRESULT f_mount (
	UCHAR vol,		/* Logical drive number to be mounted/unmounted */
...
)
{
	...
	res = chk_mounted(&path, &dj.fs, (UCHAR)(mode & ~FA_READ));
#else
	mode &= FA_READ;
	...
	for ( ;  btr;								/* Repeat until all data read */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {		/* On the sector boundary? */
			...
			if (cc) {							/* Read maximum contiguous sectors directly */
				...
				if (disk_read(fp->fs->drv, rbuff, sect, (UCHAR)cc) != RES_OK)
	...
	for ( ;  btw;							/* Repeat until all data written */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {	/* On the sector boundary? */
			...
			if (cc) {						/* Write maximum contiguous sectors directly */
				if (csect + cc > fp->fs->csize)	/* Clip at cluster boundary */
					cc = fp->fs->csize - csect;
				if (disk_write(fp->fs->drv, wbuff, sect, (UCHAR)cc) != RES_OK)
					ABORT(fp->fs, FR_DISK_ERR);
#if _FS_TINY
	...
	if (res == FR_OK) {
		...
		if (res == FR_OK) {
			...
			if (!dir) {						/* Is it a root directory? */
				...
				dir[DIR_Attr] = (value & mask) | (dir[DIR_Attr] & (UCHAR)~mask);	/* Apply attribute change */
				dj.fs->wflag = 1;
				res = sync(dj.fs);
			}
		}
...
FRESULT f_mkfs (
	UCHAR drv,		/* Logical drive number */
...
)
{
	...
	if (c < 0x80) {			/* 7-bit */
		...
		if (c < 0x800) {	/* 11-bit */
			s[0] = (UCHAR)(0xC0 | (c >> 6));
			s[1] = (UCHAR)(0x80 | (c & 0x3F));
			btw = 2;
		} else {			/* 16-bit */
			s[0] = (UCHAR)(0xE0 | (c >> 12));
			s[1] = (UCHAR)(0x80 | ((c >> 6) & 0x3F));
			s[2] = (UCHAR)(0x80 | (c & 0x3F));
			btw = 3;
		}
	}
