...
typedef struct {
	UCHAR pd;	/* Physical drive# */
	UCHAR pt;	/* Partition # (0-3) */
} PARTITION;
...
typedef struct {
	...
	UCHAR	n_fats;			/* Number of FAT copies (1,2) */
	UCHAR	wflag;			/* win[] dirty flag (1:must be written back) */
	UCHAR	fsi_flag;		/* fsinfo dirty flag (1:must be written back) */
	...
	UCHAR	win[_MAX_SS];	/* Disk access window for Directory, FAT (and Data on tiny cfg) */
} FATFS;
...
typedef struct {
	...
	UCHAR*	dir_ptr;		/* Ponter to the directory entry in the window */
#endif
#if _USE_FASTSEEK
...
#endif
#if !_FS_TINY
	UCHAR	buf[_MAX_SS];	/* File data read/write buffer */
#endif
} FIL;
...
FRESULT f_mount (UCHAR, FATFS*);						/* Mount/Unmount a logical drive */
FRESULT f_open (FIL*, const TCHAR*, UCHAR);			/* Open or create a file */
...
FRESULT f_chmod (const TCHAR*, UCHAR, UCHAR);			/* Change attriburte of the file/dir */
FRESULT f_utime (const TCHAR*, const FILINFO*);		/* Change timestamp of the file/dir */
FRESULT f_rename (const TCHAR*, const TCHAR*);		/* Rename/Move a file or directory */
FRESULT f_forward (FIL*, UINT(*)(const UCHAR*,UINT), UINT, UINT*);	/* Forward data to the stream */
FRESULT f_mkfs (UCHAR, UCHAR, UINT);					/* Create a file system on the drive */
FRESULT f_chdrive (UCHAR);							/* Change current drive */
...
/* Sync functions */
#if _FS_REENTRANT
int ff_cre_syncobj (UCHAR, _SYNC_t*);/* Create a sync object */
